let chartContainer = new SimpleTimeChart.ChartContainer("test", 1300, 70);
let chart = chartContainer.newChart(500);
let chart2 = chartContainer.newChart(200);
...
chartContainer.setTimeScale({
    startDate: new Date(data[0]["date"] * 1000),
    pixelOffset: 1300 / 20,
    deltaSecond: (data[data.length - 1]["date"] - data[0]["date"]) / 10,
    deltaPixel: 1300 / 10,
    baseDeltaPixel: 1300 / 10,
}, false);
let max = Math.max(...data.map(x => x.high));
let min = Math.max(...data.map(x => x.low));
chart.setDataScale({
    startValue: max,
    pixelOffset: 500 / 20,
    deltaValue: (min-max)/3,
    deltaPixel: 500 / 10,
    baseDeltaPixel: 500 / 10,
}, false);
chart2.setDataScale({
    startValue: max,
    pixelOffset: 200 / 20,
    deltaValue: (min-max)/3,
    deltaPixel: 200 / 10,
    baseDeltaPixel: 200 / 10,
}, false);
let candlesticks = parse(data);
let candlestickLayer = new SimpleTimeChart.CandlestickLayer(candlesticks);
chart.addLayer(candlestickLayer)

let pointList = data.map(x => {return {x: x.date * 1000, y: x.weightedAverage}});
let lineLayer = new SimpleTimeChart.LineLayer(pointList,{
    color: "#ff0",
    thickness: 2,
    opacity: 1,
})
chart2.addLayer(lineLayer)

let bandStepList = data.map(x => {return {x: x.date * 1000, top: x.high, bottom:x.low}});
let bandLayer = new SimpleTimeChart.BandLayer(bandStepList, {
    color: "#FFFFFF",
    opacity: 0.2,
});
chart2.addLayer(bandLayer);
